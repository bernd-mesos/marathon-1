...
class RichCuratorFrameworkTest extends UnitTest with StartedZookeeper {
  ...
  "RichCuratorFramework" should {
    ...
    "be able to perform transactions" in {
      richClient.create("/deleteme").futureValue
      val ops = Seq(
        richClient.createOp("/123"),
        richClient.createOp("/123/456", Some(ByteString("abc"))),
        richClient.existsOp("/123/456"),
        richClient.deleteOp("/deleteme"),
        richClient.setDataOp("/123", ByteString("123"))
      )
      val results = richClient.transaction(ops: _*).futureValue
      results.take(4) should equal(Seq(
        CreateResult("/123", "/123"),
        CreateResult("/123/456", "/123/456"),
        CheckResult("/123/456", None),
        DeleteResult("/deleteme", None)
      ))
      val setResult = results.drop(4).head.asInstanceOf[SetDataResult]
      setResult.path should equal("/123")
      setResult.stat.getVersion should equal(1)
      setResult.stat.getNumChildren should equal(1)
      setResult.stat.getDataLength should equal(ByteString("123").size)
    }
  }
}
