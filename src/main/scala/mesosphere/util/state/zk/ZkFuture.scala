...
case class CheckResult(path: String, stat: Option[Stat]) extends TransactionResult
case class CreateResult(path: String, resultingPath: String) extends TransactionResult
case class DeleteResult(path: String, resultingPath: Option[String]) extends TransactionResult
case class SetDataResult(path: String, stat: Stat) extends TransactionResult

private class TransactionFuture extends ZkFuture[Seq[TransactionResult]] {
  override protected def processEvent(event: CuratorEvent): Try[Seq[TransactionResult]] = event.getType match {
    case TRANSACTION =>
      import org.apache.curator.framework.api.transaction.OperationType._
      val results = event.getOpResults.map { result =>
        result.getType match {
          case CHECK =>
            CheckResult(result.getForPath, Option(result.getResultStat))
          case CREATE =>
            CreateResult(result.getForPath, result.getResultPath)
          case DELETE =>
            DeleteResult(result.getForPath, Option(result.getResultPath))
          case SET_DATA =>
            SetDataResult(result.getForPath, result.getResultStat)
        }
      }
      Success(results.toVector)
    case _ =>
      Failure(new IllegalArgumentException(s"${event.getType} is not a TRANSACTION operation"))
  }
}
...
private[zk] object ZkFuture {
  ...
  def transaction: ZkFuture[Seq[TransactionResult]] = new TransactionFuture
}
