...
class RichCuratorFramework(val client: CuratorFramework) extends AnyVal {
  ...
  def transaction(operations: CuratorOp*): Future[Seq[TransactionResult]] =
    build(client.transaction(), ZkFuture.transaction) { builder =>
  ...
  def createOp(
    ...
    compress: Boolean = false,
    acls: Seq[ACL] = Nil,
    createMode: CreateMode = CreateMode.PERSISTENT): CuratorOp = {
    val builder = client.transactionOp.create()
    if (compress) builder.compressed()
    if (acls.nonEmpty) builder.withACL(acls)
    builder.withMode(createMode)
    data.fold(builder.forPath(path)) { bytes =>
  ...
  def deleteOp(path: String, version: Option[Int] = None): CuratorOp = {
    val builder = client.transactionOp.delete()
    version.foreach(builder.withVersion)
    builder.forPath(path)
  }

  def setDataOp(path: String, data: ByteString,
    compress: Boolean = false,
    version: Option[Int] = None): CuratorOp = {
    val builder = client.transactionOp.setData()
    if (compress) builder.compressed()
    version.foreach(builder.withVersion)
    builder.forPath(path, data.toArray)
  }

  def existsOp(path: String, version: Option[Int] = None): CuratorOp = {
    val builder = client.transactionOp().check()
    version.foreach(builder.withVersion)
    builder.forPath(path)
  }
